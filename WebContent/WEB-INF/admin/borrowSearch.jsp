                ...
                <div class="col-lg-12">
                    <div class="panel panel-default bootstrap-admin-no-table-panel">
                        ...
                        <div class="bootstrap-admin-no-table-panel-content bootstrap-admin-panel-content collapse in">
                            <form class="form-horizontal" action="${pageContext.request.contextPath}/borrowSearch?method=queryBorrowSearchInfo&pageCode=1" method="post">
                                <div class="row">
                                
                                <div class="col-lg-5 form-group">
                                        <label class="col-lg-4 control-label" for="borrow_sno"><label class="text-danger"></label>借阅编号</label>
                                        <div class="col-lg-8">
                                            <input class="form-control"  name="borrowId" type="text" value="1" placeholder="请输入借阅编号">
                ...
                <div class="row">
                        ...
                        <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                            ...
                            <c:if test="${not empty pb }">
                             ...
                             <tbody>
	                                ...
	                                <td>${back.borrowId}</td>
	                                <td>${back.borrowInfo.book.ISBN}</td>
	                                <td>${back.borrowInfo.book.bookName}</td>
	                                <td>${back.borrowInfo.reader.paperNO}</td>
	                                <td>${back.borrowInfo.reader.name}</td>
	                                <td>${back.borrowInfo.borrowDate}</td>
	                                <td>${back.backDate }</td>
	                                <td>${back.borrowInfo.endDate}</td>
	                                <td>         
	                                	<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#findBackModal" onclick="getBackInfoById(${back.borrowId})">查看</button>
	                                	<button type="button" class="btn btn-success btn-xs" onclick="renewBook(${back.borrowId})" >续借</button>
	                               	</td>                                              
                          	  </tbody>
                           </c:forEach>
                            </c:if>
                    ...
                    <c:if test="${not empty pb }">
                        ...
                        <div class="pull-right"><!--右对齐--->
                           ...
                           <li><a href="${pageContext.request.contextPath}/borrowSearch?method=${pb.url }pageCode=1">首页</a></li>
                           <li><a href="${pageContext.request.contextPath}/borrowSearch?method=${pb.url }pageCode=${pb.pageCode-1 }">&laquo;</a></li><!-- 上一页 -->
                           <%-- 循环显示页码列表 --%>
								<c:forEach begin="${begin }" end="${end }" var="i">
								  ...
								  <%--如果是当前页则设置无法点击超链接 --%>
								  	...
								  	<c:otherwise>
								  		<li><a href="${pageContext.request.contextPath}/borrowSearch?method=${pb.url }pageCode=${i}">${i}</a></li>
								  	</c:otherwise>
								  </c:choose>
								</c:forEach>
				        	   <%--如果当前页数没到总页数，即没到最后一页,则需要显示下一页 --%>
							  <c:if test="${pb.pageCode < pb.totaPage }">
								  <li><a href="${pageContext.request.contextPath}/borrowSearch?method=${pb.url }pageCode=${pb.pageCode+1}">&raquo;</a></li>
							</c:if>
							<%--否则显示尾页 --%>
							<li><a href="${pageContext.request.contextPath}/borrowSearch?method=${pb.url }pageCode=${pb.totaPage}">尾页</a></li>
							</ul>
                           </div>
                    </c:if>           
                    </div>
