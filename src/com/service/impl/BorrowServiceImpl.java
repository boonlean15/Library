...
public class BorrowServiceImpl implements BorrowService {
	...
	@Override
	public PageBean<BorrowInfo> findBorrowInfoByPage(int pageCode, int pageSize) throws SQLException {

		PageBean<BorrowInfo> page = null;
		try {
			...
			int count = borrowInfoDao.getBorrowInfoCount();
			page.setTotalRecord(count);
			List<BorrowInfo> list = borrowInfoDao.getBorrowInfoList(pageCode, pageSize);
	...
	public BorrowInfo getBorrowInfoById(BorrowInfo info) throws SQLException {
		return borrowInfoDao.getBorrowInfoById(info);
	}

	// 添加借阅信息
	@Override
	public int addBorrow(BorrowInfo info) throws SQLException {
		/*
		 * 1. 得到借阅的读者
		 * 
		...
		// 1. 得到借阅的读者
		ReaderDao readerDao = (ReaderDao) BeanFactory.getBean("ReaderDao");
		...
		// 3. 查看该读者的借阅信息
		List<BorrowInfo> readerInfos = borrowInfoDao.getNoBackBorrowInfoByReader(reader);/// 查询的应该是未归还的记录
		// 查看借阅信息的条数
		...
		// 查看读者的罚款信息,是否有未缴纳的罚款
		ForfeitDao forfeitDao = (ForfeitDao) BeanFactory.getBean("ForfeitDao");
		...
		borrowInfo.setAid(info.getAid());
		borrowInfo.setBook(bookByISBN);
		borrowInfo.setBookId(bookByISBN.getBookId());
		borrowInfo.setBorrowDate(borrowDate);
		borrowInfo.setEndDate(endDate);
		borrowInfo.setPenalty(penalty);

		int id = borrowInfoDao.addBorrow(borrowInfo);// 返回1成功添加,返回0添加失败
		// 在馆数量需要减少
		// 添加归还信息
		int state = 0;
		if (id != 0) {
			...
			borrowInfo2.setEndDate(endDate);

			info2.setBorrowInfo(borrowInfo2);
			info2.setBorrowId(id);
			info2.setAid(borrowInfo.getAid());

			BackDao backDao = (BackDao) BeanFactory.getBean("BackDao");
	 ...
	 */
	@Override
	public int renewBook(BorrowInfo borrowInfo) throws SQLException {
		// 续借步骤：
		/*
		 * 1. 得到借阅的记录
		 * 
		 ...
		 * 
		 * 7. 得到当前借阅记录的状态 7.1 如果当前记录为逾期未归还,则需要取消当前借阅记录的罚金记录,并将该记录的状态设置为续借未归还 7.2
		 * 如果为未归还状态,直接将当前状态设置为续借未归还
		 * 
		 * 8. 为当前借阅记录进行设置,设置之后重新update该记录 返回1
		 */
		// 得到借阅的记录
		BorrowInfo borrowInfoById = borrowInfoDao.getBorrowInfoById(borrowInfo);
		// 得到借阅的记录的状态
		...
		calendar.add(Calendar.DAY_OF_MONTH, +renewDays);// +renewDays
														// ,今天的时间加renewDays天
		// 根据可续借天数,计算出截止日期
		Date endDate = calendar.getTime();
		// 设置截止时间
		...
		boolean flag = borrowInfoDao.updateBorrowInfo(borrowInfoById);
		if (flag) {
			return 1;
		}
	...
	@Override
	public boolean checkBorrowInfo() throws SQLException{
		// 检查借阅表是否有逾期
		//主要步骤
		/*
		...
		List<BorrowInfo> borrowInfos = borrowInfoDao.getBorrowInfoByNoBackState();
		if(borrowInfos!=null){
			for (BorrowInfo borrowInfo : borrowInfos) {
				...
				if(time2>time1){
					...
					forfeitInfo.setAid(borrowInfo.getAid());
